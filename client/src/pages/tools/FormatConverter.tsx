        ...
        try {
            ...
            if (ctx) {
                ...
                canvas.toBlob((blob) => {
                    ...
                    setConverting(false);
                }, targetFormat, targetFormat === 'image/png' ? undefined : quality);
            }
        } catch (error) {
            console.error(error);
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pb-12 flex flex-col max-w-4xl mx-auto">
            <div className="mb-6 flex flex-col items-center text-center">
                ...
                <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">Format Converter</h1>
                <p className="text-foreground/60 mt-2 text-sm sm:text-base">Instantly convert images to JPG, PNG, or WEBP.</p>
            </div>
                ...
                <div className="flex flex-col gap-6">
                    <div className="flex flex-col sm:flex-row gap-4 w-full">
                        <div className="flex-1 w-full">
                            <label className="block text-sm font-semibold text-foreground/80 mb-2">Target Format</label>
                            <select 
                        ...
                        {targetFormat !== 'image/png' && (
                            <div className="flex-1 w-full">
                                <label className="block text-sm font-semibold text-foreground/80 mb-2">Quality ({Math.round(quality * 100)}%)</label>
                                <input 
                                    type="range"
                                    min="0.1"
                                    max="1.0"
                    ...
                    {!file ? (
                        <div 
                            className={`flex flex-col items-center justify-center border-2 border-dashed rounded-xl p-10 min-h-[300px] cursor-pointer transition-all ${
                                dragOver ? 'border-fuchsia-500 bg-fuchsia-500/10' : 'border-border hover:border-fuchsia-500/50 hover:bg-white/5'
                            }`}
                        ...
                        >
                            ...
                            <p className="text-foreground/80 font-semibold mb-2 text-center">Click or Drop Image</p>
                            <p className="text-foreground/40 text-xs mt-1 text-center">Converts locally in your browser</p>
                        </div>
                    ) : (
                        <div className="flex flex-col border border-border rounded-xl overflow-hidden min-h-[300px] bg-black/20">
                            ...
                            <div className="flex-1 flex items-center justify-center p-4">
                                <img src={preview!} alt="Original" className="max-h-[300px] object-contain" />
                            </div>
                        </div>
                    )}
                    ...
                    >
                        ...
                        {converting ? 'Converting...' : `Convert to ${formatMap[targetFormat]}`}
                    </button>
                </div>
            </div>
            ...
            <AnimatePresence>
                {result && !converting && (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card rounded-2xl p-6 border-fuchsia-500/20 text-center shadow-xl">
                        ...
                        <div className="relative rounded-xl overflow-hidden bg-black/5 mb-6 max-h-[400px] flex items-center justify-center">
                            ...
                            <div className="absolute inset-0 z-0 opacity-20" style={{backgroundImage: 'repeating-linear-gradient(45deg, #e5e7eb 25%, transparent 25%, transparent 75%, #e5e7eb 75%, #e5e7eb), repeating-linear-gradient(45deg, #e5e7eb 25%, #ffffff 25%, #ffffff 75%, #e5e7eb 75%, #e5e7eb)', backgroundPosition: '0 0, 10px 10px', backgroundSize: '20px 20px'}}></div>
                            <img src={result.url} className="max-h-[300px] relative z-10 object-contain mx-auto" alt="Converted" />
                        </div>
                        <button 
                            onClick={() => {
                        ...
                        >
                            <Download className="w-4 h-4" /> Download {result.format.toUpperCase()}
                        </button>
                    </motion.div>
                )}
            </AnimatePresence>

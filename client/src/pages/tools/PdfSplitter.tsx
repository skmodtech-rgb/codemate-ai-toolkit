    ...
    const splitPdf = async () => {
        ...
        try {
            ...
            if (ranges.length === 1) {
                ...
                saveAs(new Blob([bytes.buffer as ArrayBuffer], { type: 'application/pdf' }), `split_${file.name}`);
            } else {
                ...
                for (let i = 0; i < ranges.length; i++) {
                    ...
                    pages.forEach(p => newPdf.addPage(p));
                    const bytes = await newPdf.save();
                    saveAs(new Blob([bytes.buffer as ArrayBuffer], { type: 'application/pdf' }), `split_part${i + 1}_${file.name}`);
                }
            }
        } catch (err) {

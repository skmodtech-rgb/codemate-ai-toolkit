...
import { LayoutGrid, Upload, Trash2, BarChart, PieChart, LineChart } from 'lucide-react';
    ...
    return (
        ...
        >
            <div className="mb-6 flex flex-col items-center text-center">
                ...
                <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">CSV Visualizer</h1>
                <p className="text-foreground/60 mt-2 text-sm sm:text-base">Upload CSV files and transform them into interactive charts.</p>
            </div>

            <div className="grid lg:grid-cols-4 gap-6">
                <div className="lg:col-span-1 space-y-6">
                    <div className="glass-card rounded-2xl p-6 border border-border shadow-xl">
                        <label className="block text-sm font-semibold text-foreground/80 mb-3">Upload CSV</label>
                        <div className="relative group">
                            ...
                            <div className="border-2 border-dashed border-border group-hover:border-amber-500/50 rounded-xl p-4 transition-all flex flex-col items-center gap-2">
                                ...
                                <span className="text-xs font-bold text-foreground/60 group-hover:text-amber-500 uppercase tracking-widest">Choose File</span>
                            </div>
                        </div>
                        ...
                        {data.length > 0 && (
                            <div className="mt-8 space-y-6">
                                <div>
                                    <label className="block text-xs font-bold text-foreground/50 uppercase tracking-widest mb-2">Chart Type</label>
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-foreground/50 uppercase tracking-widest mb-2">X-Axis (Label)</label>
                                    <select
                                        value={xAxis}
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-foreground/50 uppercase tracking-widest mb-2">Y-Axis (Value)</label>
                                    <select
                                        value={yAxis}
                    ...
                    <div className="glass-card rounded-2xl p-8 border border-border shadow-xl min-h-[500px] flex items-center justify-center bg-black/10">
                        {data.length > 0 ? (
                            ...
                            <div className="text-center space-y-4 opacity-40">
                                ...
                                <p className="text-sm">Upload a CSV file to see visualization</p>
                            </div>
                        )}
                    </div>
                </div>

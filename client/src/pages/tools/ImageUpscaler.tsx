...
import { ArrowUpCircle, Upload, Download, Loader2, Sparkles } from 'lucide-react';
...
export default function ImageUpscaler() {
    ...
    const performUpscale = async () => {
        ...
        try {
            ...
            console.error(error);
            alert('Error upscaling image. Image might be too large for local in-browser Neural Network.');
        } finally {
            setUpscaling(false);
        }
    };
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pb-12 flex flex-col max-w-4xl mx-auto">
            <div className="mb-6 flex flex-col items-center text-center">
                ...
                <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">AI Image Upscaler</h1>
                <p className="text-foreground/60 mt-2 text-sm sm:text-base">Enhance and upscale images locally using browser neural networks.</p>
            </div>
                ...
                <div className="flex flex-col gap-6">
                    {!file ? (
                        ...
                        >
                            ...
                            <p className="text-foreground/40 text-xs mt-1 text-center">Locally processed. Small images work best.</p>
                        </div>
                    ) : (
                        <div className="flex flex-col border border-border rounded-xl overflow-hidden min-h-[300px] bg-black/20">
                            <div className="h-12 border-b border-border flex items-center px-4 justify-between bg-background">
                                ...
                                <button onClick={() => { setFile(null); setPreview(null); setResultUrl(null); }} className="text-xs text-red-500 hover:text-red-400 font-semibold">Clear</button>
                            </div>
                            <div className="flex-1 flex items-center justify-center p-4">
                                <img ref={imgRef} src={preview!} alt="Original" className="max-h-[300px] object-contain" crossOrigin="anonymous" />
                            </div>
                        </div>
                    )}
                    ...
                    >
                        {upscaling ? <Loader2 className="w-5 h-5 animate-spin" /> : <Sparkles className="w-5 h-5" />}
            ...
            <AnimatePresence>
                {resultUrl && !upscaling && (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card rounded-2xl p-6 border-cyan-500/20 text-center shadow-xl">
                        ...
                        <div className="relative rounded-xl overflow-hidden bg-black/5 mb-6 max-h-[400px] flex items-center justify-center">
                            <img src={resultUrl} className="max-h-[350px] relative z-10 object-contain mx-auto rounded-lg shadow-2xl border border-border" alt="Upscaled" />
                        </div>
                        <button 

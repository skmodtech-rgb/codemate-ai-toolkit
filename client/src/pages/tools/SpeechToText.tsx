...
import { Type, Upload, FileText, Loader2, AlertCircle, Link } from 'lucide-react';
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pb-12 flex flex-col max-w-4xl mx-auto">
            <div className="mb-6">
                ...
                <p className="text-foreground/60 mt-2 text-sm sm:text-base">Transcribe audio files or voice recordings into written text.</p>
            </div>
            ...
            <div className="glass-card rounded-2xl p-6 mb-6">
                <div className="flex bg-accent border border-border rounded-xl p-1 w-max mb-6">
                    <button 
                        ...
                        className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors ${
                            inputType === 'file' ? 'bg-background shadow text-foreground' : 'text-foreground/60 hover:text-foreground'
                        }`}
                    >
                        Upload File
                    </button>
                    <button 
                        ...
                        className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors ${
                            inputType === 'url' ? 'bg-background shadow text-foreground' : 'text-foreground/60 hover:text-foreground'
                        }`}
                    >
                        Paste URL
                    </button>
                    ...
                    !file ? (
                        <div 
                            className={`border-2 border-dashed rounded-xl p-8 sm:p-12 text-center cursor-pointer transition-all ${
                                dragOver ? 'border-violet-500 bg-violet-500/10' : 'border-border hover:border-violet-500/50 hover:bg-white/5'
                            }`}
                            onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
                            onDragLeave={() => setDragOver(false)}
                            onDrop={(e) => { e.preventDefault(); setDragOver(false); if(e.dataTransfer.files[0]) handleFile(e.dataTransfer.files[0]); }}
                            onClick={() => { const input = document.createElement('input'); input.type = 'file'; input.accept = 'audio/*,video/*'; input.onchange = (e) => { const f = (e.target as HTMLInputElement).files?.[0]; if (f) handleFile(f); }; input.click(); }}
                        >
                            <Upload className="w-10 h-10 mx-auto mb-3 text-violet-500/60" />
                            <p className="text-foreground/80 font-semibold mb-1">Drop audio/video file here</p>
                            <p className="text-foreground/40 text-xs">MP3, WAV, MP4, M4A up to 50MB</p>
                        </div>
                    ) : (
                        <div className="flex items-center justify-between p-4 bg-background border border-border rounded-xl">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-semibold truncate max-w-[200px] sm:max-w-[400px]">{file.name}</p>
                                    <p className="text-xs text-foreground/50">Ready to transcribe</p>
                                </div>
                            </div>
                            <button onClick={() => setFile(null)} className="text-sm font-semibold text-red-500 hover:text-red-400">
                                Remove
                            </button>
                        </div>
                    )
                ...
                >
                    {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Type className="w-5 h-5" />}
            ...
            <AnimatePresence>
                {resultText && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-2xl p-6 border-violet-500/20">

...
import { ShieldCheck, Eye, EyeOff, AlertTriangle, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function PasswordStrength() {
    ...
    const getScoreIcon = () => {
        if (!result) return <ShieldCheck className="w-12 h-12 text-foreground/20" />;
        if (result.score <= 1) return <AlertTriangle className="w-12 h-12 text-red-500" />;
        if (result.score === 2) return <AlertCircle className="w-12 h-12 text-orange-500" />;
        if (result.score === 3) return <CheckCircle2 className="w-12 h-12 text-yellow-500" />;
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pb-12 flex flex-col max-w-2xl mx-auto">
            <div className="mb-6 flex flex-col items-center text-center">
                ...
                <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">Password Strength</h1>
                <p className="text-foreground/60 mt-2 text-sm sm:text-base">Check the strength and hackability of your password.</p>
            </div>
            ...
            <div className="glass-card rounded-2xl p-6 sm:p-8 mb-6 shadow-xl border border-border">
                ...
                <div className="flex flex-col items-center justify-center gap-4 mb-8">
                    ...
                    <h2 className={`text-3xl font-black uppercase tracking-widest ${
                        !result ? 'text-foreground/20' : 
                        result.score <= 1 ? 'text-red-500' :
                        result.score === 2 ? 'text-orange-500' :
                        result.score === 3 ? 'text-yellow-500' :
                        'text-emerald-500'
                    }`}>
                        {getStrengthLabel()}
                    </h2>
                    ...
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-black/5 rounded-xl border border-border p-4 text-center flex flex-col gap-2">
                                <span className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Estimated Hack Time</span>
                                <span className={`text-xl font-black ${result.score > 2 ? 'text-emerald-500' : 'text-red-500'}`}>{formatCrackTime()}</span>
                            </div>
                            <div className="bg-black/5 rounded-xl border border-border p-4 text-center flex flex-col gap-2">
                                <span className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Guesses to Crack</span>
                                <span className="text-xl font-black text-indigo-500">{result.guesses.toLocaleString()}</span>
                            </div>
                        </div>

                        {/* Feedback */}
                        {result.feedback && (result.feedback.warning || result.feedback.suggestions.length > 0) && (

    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pb-12 flex flex-col max-w-5xl mx-auto">
            <div className="mb-6">
                ...
                <p className="text-foreground/60 mt-2 text-sm sm:text-base">Crop images freehand or using specific aspect ratios.</p>
            </div>
            ...
            <div className="grid lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1 border border-border bg-background rounded-2xl p-6 relative flex flex-col gap-6">
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-foreground mb-3">Aspect Ratio</label>
                        <div className="flex flex-wrap gap-2">
                            <button onClick={() => handleAspectChange(undefined)} className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors ${aspect === undefined ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-accent/50 text-foreground/60 hover:text-foreground'}`}>Free</button>
                            <button onClick={() => handleAspectChange(1)} className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors ${aspect === 1 ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-accent/50 text-foreground/60 hover:text-foreground'}`}>1:1 (Square)</button>
                            <button onClick={() => handleAspectChange(16 / 9)} className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors ${aspect === 16/9 ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-accent/50 text-foreground/60 hover:text-foreground'}`}>16:9</button>
                            <button onClick={() => handleAspectChange(4 / 3)} className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors ${aspect === 4/3 ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-accent/50 text-foreground/60 hover:text-foreground'}`}>4:3</button>
                        </div>
                    </div>
                    ...
                    >
                        {cropping ? <Loader2 className="w-4 h-4 animate-spin" /> : <CropIcon className="w-4 h-4" />}
                    ...
                    {!file ? (
                        <div 
                            className={`glass-card rounded-2xl border-2 border-dashed p-12 text-center cursor-pointer transition-all flex flex-col items-center justify-center min-h-[400px] ${
                                dragOver ? 'border-emerald-500 bg-emerald-500/10' : 'border-border hover:border-emerald-500/50 hover:bg-white/5'
                            }`}
                        ...
                        <div className="glass-card rounded-2xl flex flex-col overflow-hidden min-h-[400px]">
                            <div className="h-14 border-b border-border flex items-center px-4 justify-between bg-accent/30">
                                ...
                                <div className="flex gap-4">
                                    {resultUrl && <button onClick={() => setResultUrl(null)} className="text-xs text-emerald-500 hover:text-emerald-400 font-semibold">Back to Edit</button>}
                                    <button onClick={() => { setFile(null); setPreview(null); setResultUrl(null); }} className="text-xs text-red-500 hover:text-red-400 font-semibold">Change File</button>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 p-6 flex flex-col items-center justify-center bg-[url('https://grainy-gradients.vercel.app/noise.svg')] bg-[length:100px_100px] opacity-[0.98] overflow-hidden">
                                {resultUrl ? (
                                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center w-full gap-6">
                                        <div className="relative rounded-xl overflow-hidden shadow-2xl max-w-full bg-black/5 flex items-center justify-center">
                                            <img src={resultUrl} alt="Cropped" className="max-h-[350px] object-contain block" />
                                        </div>
                                        <button 

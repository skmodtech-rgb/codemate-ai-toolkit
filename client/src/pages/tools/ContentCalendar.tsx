import { CalendarDays, ChevronLeft, ChevronRight, Share2, MessageSquare, ListTree, Hash, Plus } from 'lucide-react';
        ...
        for (let i = 1; i <= 31; i++) {
            ...
            calendar.push(
                <div key={i} className={cn(
                    ...
                    isToday ? "bg-brand-500/5 group-hover:bg-brand-500/10" : "bg-background/20 hover:bg-background/50",
        ...
        <div className="max-w-6xl mx-auto pb-10">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">Bird's eye view of your scheduled and published posts.</p>
                    </div>
                </div>
                
                <div className="flex flex-wrap items-center gap-3">
                    <button className="btn-secondary px-4 py-2 text-sm rounded-xl">Today</button>
                    <div className="flex items-center glass-card rounded-xl border border-border/50 p-1">
                        ...
                        <span className="text-sm font-semibold text-foreground px-4">March 2026</span>
                        <button className="p-1.5 hover:bg-background/80 rounded-lg text-muted-foreground hover:text-foreground transition-colors">
            ...
            <div className="glass-card rounded-3xl overflow-hidden border border-border/50 shadow-sm">
                <div className="grid grid-cols-7 border-b border-border/50 bg-background/40 p-3">
                    {days.map(d => (
                        <div key={d} className="text-center text-xs font-bold text-muted-foreground uppercase tracking-widest">{d}</div>
                    ))}
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8">
                {[
                    ...
                    { label: 'Instagram', color: 'bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-500' },
                    { label: 'Facebook', color: 'bg-[#1877F2]' },
                ].map((item, i) => (
                    <div key={i} className="glass-card p-3 rounded-xl flex items-center justify-center gap-2 border border-border/30">

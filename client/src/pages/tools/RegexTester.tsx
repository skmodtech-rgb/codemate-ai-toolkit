...
export default function RegexTester() {
    ...
    const [testString, setTestString] = useState('My emails are test@example.com and dev@toolmate.ai');
    ...
    return (
        ...
        >
            <div className="mb-6 flex flex-col items-center text-center">
                ...
                <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">Regex Tester</h1>
                <p className="text-foreground/60 mt-2 text-sm sm:text-base">Real-time regular expression testing and debugging suite.</p>
            </div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-6">
                    <div className="glass-card rounded-2xl p-6 border border-border shadow-xl space-y-4 bg-black/5">
                        <div className="flex gap-4">
                            <div className="flex-1 space-y-2">
                                <label className="text-xs font-bold text-foreground/50 uppercase tracking-widest">Regular Expression</label>
                                <div className="relative">
                                    ...
                                    />
                                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-indigo-500 font-bold">/</span>
                                </div>
                            </div>
                            <div className="w-24 space-y-2">
                                <label className="text-xs font-bold text-foreground/50 uppercase tracking-widest">Flags</label>
                                <input
                                    type="text"
                                    value={flags}
                    ...
                    <div className="glass-card rounded-2xl p-6 border border-border shadow-xl space-y-4 bg-black/5">
                        <label className="text-xs font-bold text-foreground/50 uppercase tracking-widest">Test String</label>
                        <textarea
                            value={testString}
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="glass-card rounded-2xl p-6 border border-border shadow-xl h-full flex flex-col bg-black/5">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-sm font-bold text-foreground/50 uppercase tracking-widest">Match Results</h3>
                            <span className="bg-indigo-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full">{result.matches.length} Matches</span>
                        </div>

                        <div className="flex-1 space-y-4 overflow-y-auto max-h-[600px] pr-2 scrollbar-hide">
                            {result.matches.length > 0 ? (
                                result.matches.map((match, i) => (
                                    <div key={i} className="bg-black/20 border border-border rounded-xl p-4 space-y-3">
                                        <div className="flex items-center justify-between border-b border-border/50 pb-2">
                                            <span className="text-[10px] font-black text-indigo-500 uppercase tracking-tighter">Match #{i + 1}</span>
                                            <span className="text-[10px] text-foreground/40 font-mono">Index: {match.index}</span>
                                        </div>
                                        <div className="text-sm font-mono text-emerald-400 break-all">{match[0]}</div>
                                        {match.length > 1 && (
                                            <div className="space-y-1 pt-2">
                                                <p className="text-[9px] font-bold text-foreground/30 uppercase tracking-widest">Captured Groups</p>
                                                {Array.from(match).slice(1).map((group, gi) => (
                                                    <div key={gi} className="text-[11px] font-mono text-indigo-400 bg-indigo-500/5 px-2 py-1 rounded border border-indigo-500/10 mb-1">
                                                        ${gi + 1}: {group}
                                                    </div>
                                                ))}
                                            </div>

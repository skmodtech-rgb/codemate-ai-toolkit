...
export default function PdfCompressor() {
    ...
    const savings = file && result ? Math.max(0, ((file.size - result.size) / file.size * 100)) : 0;
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pb-12 flex flex-col">
            <div className="mb-6">
                ...
                <p className="text-foreground/60 mt-2 text-sm sm:text-base">Reduce PDF file size by optimizing internal structure and removing metadata.</p>
            </div>
                    ...
                    <>
                        <div className="flex items-center justify-between p-4 rounded-xl bg-background/50 border border-border">
                            <div>
                                ...
                                <p className="text-xs text-foreground/40">Original: {formatSize(file.size)}</p>
                            </div>
                            <button onClick={() => { setFile(null); setResult(null); }} className="text-sm text-red-400 hover:text-red-300">Change File</button>
                        </div>
                            ...
                            <>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    <div className="p-4 rounded-xl bg-background/50 border border-border text-center">
                                        <p className="text-xs text-foreground/50 uppercase tracking-wider mb-1">Original</p>
                                        <p className="text-xl font-bold text-foreground">{formatSize(file.size)}</p>
                                    </div>
                                    <div className="p-4 rounded-xl bg-background/50 border border-brand-500/30 text-center">
                                        <p className="text-xs text-foreground/50 uppercase tracking-wider mb-1">Compressed</p>
                                        <p className="text-xl font-bold text-brand-400">{formatSize(result.size)}</p>
                                    </div>
                                    <div className="p-4 rounded-xl bg-background/50 border border-emerald-500/30 text-center">
                                        <p className="text-xs text-foreground/50 uppercase tracking-wider mb-1">Saved</p>
                                        <p className="text-xl font-bold text-emerald-400">{savings.toFixed(1)}%</p>
                                    </div>
                                </div>

                                {savings < 1 && (
                                    <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-400 text-sm">
                                        ⚠️ This PDF is already well-optimized. Client-side compression has limited effect on image-heavy PDFs.
                                    </div>
                                )}

                                <button onClick={downloadCompressed} className="btn-primary w-full h-12 gap-2 text-base">

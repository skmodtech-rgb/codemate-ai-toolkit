...
export default function PdfToWord() {
    ...
    const downloadAsDocx = async () => {
        ...
        extractedText.forEach((text, idx) => {
            children.push(
                new Paragraph({
                    children: [new TextRun({ text: `— Page ${idx + 1} —`, bold: true, size: 28 })],
    ...
    const downloadAsTxt = () => {
        const text = extractedText.map((t, i) => `--- Page ${i + 1} ---\n${t}`).join('\n\n');
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pb-12 flex flex-col">
            <div className="mb-6">
                ...
                <p className="text-foreground/60 mt-2 text-sm sm:text-base">Extract text from PDF and convert it into a Word (.docx) document.</p>
            </div>
            ...
            <div className="glass-card rounded-2xl p-4 sm:p-6 w-full max-w-4xl mx-auto flex flex-col gap-6">
                ...
                {!file ? (
                    <div 
                        ...
                        onClick={() => { const input = document.createElement('input'); input.type = 'file'; input.accept = '.pdf'; input.onchange = (e) => { const f = (e.target as HTMLInputElement).files?.[0]; if (f) handleFile(f); }; input.click(); }}
                    >
                        ...
                        <p className="text-foreground/40 text-sm mt-1">Text-based PDFs work best</p>
                    </div>
                ) : (
                    <>
                        <div className="flex items-center justify-between p-4 rounded-xl bg-background/50 border border-border">
                            ...
                            <button onClick={() => { setFile(null); setExtractedText([]); setStep('upload'); }} className="text-sm text-red-400 hover:text-red-300">Change File</button>
                        </div>
                        ...
                        {step === 'upload' && (
                            ...
                            >
                                {converting ? (
                                    <><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Extracting text...</>
                                ) : (
                        ...
                        {step === 'preview' && extractedText.length > 0 && (
                            <>
                                <div className="flex flex-col gap-2">
                                    <h3 className="text-sm font-semibold text-foreground/80">Preview ({extractedText.length} pages extracted)</h3>
                                    <div className="bg-background border border-border rounded-xl p-4 max-h-[400px] overflow-y-auto text-sm text-foreground/80 whitespace-pre-wrap font-mono">
                                        {extractedText.map((text, idx) => (
                                            <div key={idx} className="mb-4">
                                                <p className="text-brand-500 font-bold mb-1">— Page {idx + 1} —</p>
                                                <p>{text || '(No text found on this page — it may be image-based)'}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <button onClick={downloadAsDocx} className="btn-primary h-12 gap-2">
                                        <Download className="w-5 h-5" /> Download .docx
                                    </button>
                                    <button onClick={downloadAsTxt} className="btn-secondary h-12 gap-2">
                        ...
                        {step === 'done' && (
                            <div className="text-center py-8">
                                ...
                                <p className="text-foreground/60 text-sm">Your Word document has been downloaded.</p>
                                <button onClick={() => { setFile(null); setExtractedText([]); setStep('upload'); }} className="mt-4 text-brand-400 hover:text-brand-300 text-sm font-semibold">Convert another PDF</button>
                            </div>
                        )}
                    </>
                )}
